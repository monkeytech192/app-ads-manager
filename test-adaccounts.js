...
async function testAdAccounts() {
  try {
    ...
    try {
      const response1 = await fetch(`https://graph.facebook.com/v24.0/me/adaccounts?fields=id,name,account_id,account_status&access_token=${FACEBOOK_ACCESS_TOKEN}`);
    ...
    try {
      ...
      const response2 = await fetch(`https://graph.facebook.com/v24.0/${meData.id}/adaccounts?fields=id,name,account_id,account_status&access_token=${FACEBOOK_ACCESS_TOKEN}`);
    ...
    try {
      const bizResponse = await fetch(`https://graph.facebook.com/v24.0/me/businesses?fields=id,name&access_token=${FACEBOOK_ACCESS_TOKEN}`);
      ...
      if (bizData.data && bizData.data.length > 0) {
        ...
        const response3 = await fetch(`https://graph.facebook.com/v24.0/${businessId}/owned_ad_accounts?fields=id,name,account_id,account_status&access_token=${FACEBOOK_ACCESS_TOKEN}`);
